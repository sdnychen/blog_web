...
<template>
    <div class="carousel-box" @mouseenter="handleHeanMouseEnter" @mouseleave="handleHeanMouseLeave">
        <div class="carousel-item" :style="{ 'right': `${currPageIndex * 100}%` }">
            <img v-for="(item) in dataList" :key="item.uid" class="carousel-img" :src="item.src" :alt="item.alt"
        ...
        <div v-if="showDots" class="dots">
            <div v-for="(item, index) in dataList" :key="item.uid"
                :class="['dot', currPageIndex === index ? 'dot-active' : '']" @click="handleChangeDot(index)" />
        </div>
    </div>
</template>
